...
export default function CertificationsScreen() {
  ...
  // When editItem is set and employees/certTypes are loaded, seed the FK fields from the loaded lists
  useEffect(() => {
    if (!editItem || employees.length === 0 || certTypes.length === 0) return;
    // Match by name since the record only has the joined name, not the raw FK id
    const matchedEmp  = employees.find(e => e.name === editItem.employee?.name);
    const matchedType = certTypes.find(t => t.name === editItem.cert_type?.name);
    setDraft(d => ({
      ...d,
      employee_id:  matchedEmp  ? matchedEmp.id  : d.employee_id,
      cert_type_id: matchedType ? matchedType.id : d.cert_type_id,
    }));
  }, [editItem, employees, certTypes]);
  ...
  const handleDelete = () => {
    ...
    Alert.alert(
      ...
      `Remove ${editItem.cert_type?.name ?? 'this certification'} for ${editItem.employee?.name ?? 'this employee'}? This cannot be undone.`,
      [
...
const f = StyleSheet.create({
  ...
  deleteBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 36, paddingVertical: 14, borderRadius: 8, borderWidth: 1, borderColor: colors.red + '55', backgroundColor: colors.red + '11' },
